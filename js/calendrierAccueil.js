function include(fileName){
	document.write("<script type='text/javascript'"+fileName+"></script>");
}
...
//au début c'est aujourd'hui comme today
var xmlHttp;
var today = new Date();
...
btnNextMonth.addEventListener("click", function() {
    ...
    currentDate.setMonth( ++currentMonth);
    newCalendar();
}, false);//peut etre il y a un probleme de fonction bouton
...
//var dateInfo = currentDate.toLocaleDateString();
	
function sendURL(dateInfo){
	document.location.href="calendrier.php?info="+dateInfo;
}
 	...
 	for (var i = 0; i < 6; i++){
		...
		for(var j = 0; j < 7; j++){
			...
			if(month>mois&&(nextMonth-mois)<3){
				...
				newDate.setAttribute("id",dateInfo);
				newDate.setAttribute("class", "date");
				newDate.setAttribute("onmousedown", "generateToday(\"" + dateInfo+ "\")");
				//newDate.setAttribute("onclick","sendURL()");
				newRow.appendChild(newDate);
			}
			if((nextMonth-mois)==3){
				newDate.innerText="";
			...
			//ici il faut regler le probleme de selectionner la date
		}
...
function generateToday(dateString) {
    if(dateString){ //si il transmet l'argument, il mettre les valeurs selon l'argument
        ...
        if( currentDate.getDate() > 30) {
            ...
            currentDate.setMonth(parseInt(info[1])-1 );
            currentDate.setDate(info[0]);
        }//peut etre il faut mettre dateString
    }
    if(datedDate == null){
    ...
    var dateInfo = currentDate.toLocaleDateString(); //obtenir la date en format jj/mm/aaaa
	...
	//dateTd.setAttribute("onload","sendURL()");
